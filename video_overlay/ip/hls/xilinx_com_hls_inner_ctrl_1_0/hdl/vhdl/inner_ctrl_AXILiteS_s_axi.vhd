...
entity inner_ctrl_AXILiteS_s_axi is
generic (
...
end entity inner_ctrl_AXILiteS_s_axi;
...
architecture behave of inner_ctrl_AXILiteS_s_axi is
    ...
    constant ADDR_OUTPUT_V_DATA_0 : INTEGER := 16#18#;
    constant ADDR_OUTPUT_V_CTRL   : INTEGER := 16#1c#;
...
-- ----------------------- Register logic ----------------
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                int_input_V <= (others => '0');
            elsif (ACLK_EN = '1') then
                if (input_V_ap_vld = '1') then
                    int_input_V <= UNSIGNED(input_V); -- clear on read
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ARESET = '1') then
                ...
                elsif (ar_hs = '1' and raddr = ADDR_INPUT_V_CTRL) then
                    int_input_V_ap_vld <= '0'; -- clear on read
                end if;
            end if;
        end if;
    end process;
